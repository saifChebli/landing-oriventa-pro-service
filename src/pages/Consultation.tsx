...
export default function Consultation() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:5000/api/consultations/add-consultation", form);
  ...
  return (
    <div style={{ direction: "ltr" }} translate="no" className="min-h-screen font-kufam bg-gradient-to-b from-slate-50 to-white text-slate-800">
      <TopBar />
          ...
          <div>
            ...
            <section className="bg-white rounded-2xl shadow-xl/40 max-w-4xl mx-auto shadow-slate-200 p-6 sm:p-8 border border-slate-100">
              <form onSubmit={handleSubmit} className="mt-6 gap-4">
                ...
                <Field labelFR="Emploi / Domaine actuel" labelAR="عملك / مجالك الحالي" required>
                  <input
                    type="text"
                    name="jobDomain"
                ...
                <Field labelFR="Pays de destination" labelAR="البلد المراد الهجرة إليه" required>
                  <input
                    type="text"
                    name="destination"
                ...
                <Field labelFR="Type de travail souhaité" labelAR="نوع الوظيفة المرغوبة" required>
                  <input
                    type="text"
                    name="jobType"
                    value={form.jobType}
                ...
                <Field labelFR="Pourquoi travailler à l'étranger ?" labelAR="لماذا تريد العمل في الخارج؟" required>
                  <textarea
                    name="reason"
                    value={form.reason}
                    onChange={handleChange}
                ...
                <Field labelFR="Informations complémentaires" labelAR="معلومات إضافية" required>
                  <textarea
                    name="extra"
                    value={form.extra}
                    onChange={handleChange}
    ...
    <header className="px-4 sm:px-6 lg:px-8 pt-10 sm:pt-14">
      <div className="flex flex-col justify-center items-center max-w-4xl mx-auto rounded-3xl border border-slate-100 bg-white p-6 sm:p-10 shadow-lg/40 shadow-slate-200">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-1 text-amber-500 text-2xl" aria-label="5 stars">
            <span>⭐️</span>
            <span>⭐️</span>
            <span>⭐️</span>
            <span>⭐️</span>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-6 sm:gap-10">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-3 text-slate-600 max-w-prose text-center">
              Remplissez le formulaire et notre équipe vous contactera rapidement pour évaluer votre profil, vos objectifs, et la meilleure voie pour travailler à l'étranger.
            </p>
          </div>
        </div>
      </div>
...
function Field({ labelFR, labelAR, required, className, children }: FieldProps) {
  return (
    <label className={`block ${className || ""}`}>
      <span className="block text-lg my-8 font-medium text-slate-700">
        {labelFR} - {labelAR} {required && <span className="text-rose-600">*</span>}
      </span>
